...
def populate(N=5):
    for entry in range(N):
        ...
        match_performance_obj=match_performance.objects.get_or_create(match_id=match_id,player_id=player_id,runs=runs,catches=catches,wickets=wickets)[0]

        user_team_obj=user_team.objects.get_or_create(user_id=user_id,match_id=match_id)[0]
